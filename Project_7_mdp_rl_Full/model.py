...
class DeepQNetwork():
    ...
    def run(self, states):
        ...
        """
        "*** YOUR CODE HERE ***"
        output1 = nn.ReLU(nn.AddBias(nn.Linear(states, self.w1), self.b1))
        output2 = nn.ReLU(nn.AddBias(nn.Linear(output1, self.w2), self.b2))
        output3 = nn.ReLU(nn.AddBias(nn.Linear(output2, self.w3), self.b3))
        output4 = nn.ReLU(nn.AddBias(nn.Linear(output3, self.w4), self.b4))
        output5 = nn.AddBias(nn.Linear(output4, self.w5), self.b5)
