...
class PacmanDeepQAgent(PacmanQAgent):
    def __init__(self, layout_input="smallGrid", target_update_rate=300, doubleQ=True, **args): # original doubleQ=True, **args
        ...
        self.epsilon0 = 0.05     # original 0.05
        self.epsilon = self.epsilon0
        self.discount = 0.9
        self.update_frequency = 3   # original 3
        self.counts = None
        self.replay_memory = ReplayMemory(50000)
        self.min_transitions_before_training = 10000 # original 10000
        ...
        '''
        if isinstance(layout_input, str):
            self.layout_instance = layout.getLayout(layout_input) # <--- 添加这行
        else:
            self.layout_instance = layout_input # <--- 添加这行
        self.state_dim = self.get_state_dim(self.layout_instance) # <--- 使用 self.layout_instance
        self.initialize_q_networks(self.state_dim)
        '''
    ...
    def get_features(self, state):
        # 获取地图的宽度和高度，用于归一化
        # 确保 self.layout_instance 已经在 __init__ 中被设置
        width = self.layout_instance.width
        height = self.layout_instance.height

        # Pacman 位置归一化
        pacman_pos = state.getPacmanPosition()
        # 减 1 是因为坐标通常是 0 到 max-1
        norm_pacman_x = pacman_pos[0] / (width - 1.0) if width > 1 else pacman_pos[0]
        norm_pacman_y = pacman_pos[1] / (height - 1.0) if height > 1 else pacman_pos[1]
        pacman_features = np.array([norm_pacman_x, norm_pacman_y])

        # Ghost 位置归一化
        ghost_features = []
        ghost_positions = state.getGhostPositions()
        for g_pos in ghost_positions:
            norm_ghost_x = g_pos[0] / (width - 1.0) if width > 1 else g_pos[0]
            norm_ghost_y = g_pos[1] / (height - 1.0) if height > 1 else g_pos[1]
            ghost_features.extend([norm_ghost_x, norm_ghost_y])
        ghost_features = np.array(ghost_features).flatten()

        # 食物和胶囊特征 (保持原有逻辑，因为它们的值范围已经很小)
        food_locations = np.array(state.getFood().data).astype(np.float32)
        capsules = state.getCapsules()
        for x, y in capsules:
            food_locations[x][y] = 2.0 # 确保是浮点数，保持与原始逻辑一致

        return np.concatenate((pacman_features, ghost_features, food_locations.flatten()))
    '''
