...
def pacphysics_axioms(t, all_coords, non_outer_wall_coords):
    ...
    for x, y in non_outer_wall_coords:
        temp1.append(PropSymbolExpr(pacman_str, x, y, t))
    pacphysics_sentences.append(exactlyOne(temp1))

    temp2 = []
    for i in DIRECTIONS:
        temp2.append(PropSymbolExpr(i, t))
    pacphysics_sentences.append(exactlyOne(temp2))
...
def positionLogicPlan(problem):
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            temp1.append(PropSymbolExpr(pacman_str, x, y, t))
        KB.append(exactlyOne(temp1))

        goal_assertion = PropSymbolExpr(pacman_str, xg, yg, t)
        solution = findModel(conjoin(KB) & goal_assertion)
        if solution:
...
def foodLogicPlan(problem):
    ...
    for t in range(50):
        ...
        temp3 = []
        for x, y in food:
            temp_expr1 = PropSymbolExpr(food_str, x, y, t)
            temp3.append(~temp_expr1)

            temp_expr2 = (PropSymbolExpr(pacman_str, x, y, t) & temp_expr1)
            temp_expr3 = ~PropSymbolExpr(food_str, x, y, t + 1)
            KB.append((temp_expr2 % temp_expr3) | ~temp_expr1)
        empty = conjoin(temp3)
        if findModel(empty & conjoin(KB)):
            return extractActionSequence(findModel(empty & conjoin(KB)), actions)
...
def localization(problem, agent):
    ...
    for t in range(agent.num_timesteps):
        ...
        temp = agent.getPercepts()
        KB.append(four_bit_percept_rules(t, temp))

        possible_locations_t = []
        for x, y in non_outer_wall_coords:
            expr = PropSymbolExpr(pacman_str, x, y, t)
            res1 = findModel(conjoin(KB) & expr)
            res2 = findModel(conjoin(KB) & ~expr)
            if res1:
...
def mapping(problem, agent):
    ...
    for t in range(agent.num_timesteps):
        ...
        updated_wall = [[] for i in range(len(known_map))]
        for i in range(len(known_map)):
            for j in range(len(known_map[i])):
                temp = known_map[i][j]
                updated_wall[i].append(temp if temp != -1 else 0)

        KB.append(allLegalSuccessorAxioms(t + 1, updated_wall, non_outer_wall_coords))
...
def slam(problem, agent):
    ...
    for t in range(agent.num_timesteps):
        ...
        temp = agent.getPercepts()
        KB.append(num_adj_walls_percept_rules(t, temp))
        ...
        KB.append(SLAMSuccessorAxioms(t + 1, updated_wall, non_outer_wall_coords))
