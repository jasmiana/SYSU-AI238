...
def constructBayesNet(gameState):
    ...
    edges = []
    variableDomainsDict = {}
    # Hint: 查看 bayesNet.py 里的 printStarterBayesNet()里调用的 constructEmptyBayesNet 函数的参数来定义吃豆人游戏贝叶斯网络的参数
    "*** YOUR CODE HERE ***"
    ...
    for i in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(i):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(obsVar)
            variableDomainsDict[obsVar] = OBS_VALS

    for i in obsVars:
        edges.append((FOOD_HOUSE_VAR, i))
        edges.append((GHOST_HOUSE_VAR, i))
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    prob = 0
    for asgn in query_factor.getAllPossibleAssignmentDicts():
        temp_prob = query_factor.getProbability(asgn)
        if temp_prob > prob:
            prob = temp_prob
...
class VPIAgent(BayesAgent):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for assignment in query_factor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                left_food = query_factor.getProbability(assignment)
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                right_food = query_factor.getProbability(assignment)

        leftExpectedValue = left_food * WON_GAME_REWARD + (1 - left_food) * GHOST_COLLISION_REWARD
        rightExpectedValue = right_food * WON_GAME_REWARD + (1 - right_food) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, evd in self.getExplorationProbsAndOutcomes(evidence):
            new_evd = evidence.copy()
            new_evd.update(evd)
            left, right = self.computeEnterValues(new_evd, enterEliminationOrder)
